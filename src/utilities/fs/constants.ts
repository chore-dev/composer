...
const directoryResolverBuilder = (root: string | RootFn) => (path = './') => resolve(typeof root === 'function' ? root() : root, path);
...
export const datetimeDirectory = directoryResolverBuilder(() => backupDirectory(getApplication().datetime));

export const templateDirectory = (folder: keyof Templates, file: Templates[keyof Templates]) => dist(`../src/sections/${folder}/templates/${file}`);
